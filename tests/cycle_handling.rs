...
fn impl_where_clause_cannot_justify_its_matching_supertrait() {
    ...
    .err(expect_test::expect![[r#"
        ...
        crates/formality-rust/src/prove/prove/prove/prove_validate.rs:16:1: no applicable rules for prove_validate { validation_state: a, validate_goal: Prerequisite(Ground), assumptions: {validate(a, Magic(Ground))}, env: Env { variables: [], bias: Soundness, pending: [], allow_pending_outlives: true } }"#]]);
}
...
fn associated_type_ensures_cycle_cannot_invent_missing_supertrait_impl() {
    ...
    .err(expect_test::expect![[r#"
        ...
        crates/formality-rust/src/prove/prove/prove/prove_validate.rs:16:1: no applicable rules for prove_validate { validation_state: a, validate_goal: PartialOrd(Bad), assumptions: {PartialOrd(<u32 as Family>::Gat<Bad>), validate(a, Ord(Bad))}, env: Env { variables: [], bias: Soundness, pending: [], allow_pending_outlives: false } }"#]]);
}

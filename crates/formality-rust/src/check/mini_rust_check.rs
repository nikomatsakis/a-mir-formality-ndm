...
judgment_fn! {
    fn check_value(
    ...
    ) => (Ty, TypeckEnv, Set<PendingOutlives>) {
        ...
        (
            ...
            --- ("constant")
            (check_value(env, outlives, _fn_assumptions, Constant(constant)) => (value_ty, env, outlives))
        )
...
impl TypeckEnv {
    ...
    pub(crate) fn check_place_hackola(
        ...
        let ((ty, _env, new_outlives), _pt) = check_place(
            self.clone(),
            outlives.clone(),

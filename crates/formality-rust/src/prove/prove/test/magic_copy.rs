...
fn all_t_is_not_magic_without_copy_for_all_t() {
    ...
    test_prove(decls(), term("{} => {for<T> T: Magic}")).assert_err(expect![[r#"
        ...
        crates/formality-rust/src/prove/prove/prove/prove_via_impl.rs:51:1: no applicable rules for prove_via_impl { requested_trait_ref: !ty_1: Magic, candidate: ImplCandidate { id: ImplId { crate_index: 0, item_index: 2 }, trait_impl: impl <ty> Magic for ^ty0_0 where ^ty0_0 : Magic { } }, assumptions: {}, env: Env { variables: [!ty_1], bias: Soundness, pending: [], allow_pending_outlives: false } }"#]]);
}
...
fn invalid_magic_impl_does_not_imply_copy_for_all_t() {
    ...
    test_prove(decls(), term("{} => {for<T> T: Copy}")).assert_err(expect![[r#"
        ...
        crates/formality-rust/src/prove/prove/prove/prove_via_impl.rs:51:1: no applicable rules for prove_via_impl { requested_trait_ref: !ty_1: Copy, candidate: ImplCandidate { id: ImplId { crate_index: 0, item_index: 3 }, trait_impl: impl Copy for u32 { } }, assumptions: {}, env: Env { variables: [!ty_1], bias: Soundness, pending: [], allow_pending_outlives: false } }"#]]);
}

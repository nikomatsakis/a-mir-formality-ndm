...
fn all_t_is_not_magic_without_copy_for_all_t() {
    ...
    test_prove(decls(), term("{} => {for<T> Magic(T)}")).assert_err(expect![[r#"
        ...
        crates/formality-rust/src/prove/prove/prove/prove_wc.rs:329:1: no applicable rules for prove_validate { validation_state: a, validate_goal: Copy(!ty_0), assumptions: {validate(a, Magic(!ty_0))}, env: Env { variables: [!ty_0], bias: Soundness, pending: [], allow_pending_outlives: false } }"#]]);
}

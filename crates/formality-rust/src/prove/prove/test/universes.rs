...
fn exists_u_for_t() {
    ...
    test_prove(decls, term("exists<U> {} => {for<T> T = U}")).assert_err(expect![[r#"
        crates/formality-rust/src/prove/prove/prove/prove_normalize.rs:51:1: no applicable rules for prove_normalize_now { p: !ty_1, assumptions: {}, env: Env { variables: [?ty_0, !ty_1], bias: Soundness, pending: [], allow_pending_outlives: false } }
        ...
        crates/formality-rust/src/prove/prove/prove/prove_normalize.rs:51:1: no applicable rules for prove_normalize_now { p: ?ty_0, assumptions: {}, env: Env { variables: [?ty_0, !ty_1], bias: Soundness, pending: [], allow_pending_outlives: false } }"#]]);
}

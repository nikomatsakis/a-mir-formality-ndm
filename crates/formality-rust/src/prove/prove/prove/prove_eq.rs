...
use formality_core::{judgment_fn, judgment::FailureLocation, Downcast, ProvenSet, Upcast};
...
fn equate_variable(
    ...
    if occurs_in(x, &fvs) {
        return ProvenSet::failed("equate_variable", FailureLocation::caller(), format!("`{x:?}` occurs in `{p:?}`"));
    }
